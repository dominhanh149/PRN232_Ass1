...
namespace Assignmen_PRN232_1.Controllers.Api
{
    ...
    public class TagController : ControllerBase
    {
        ...
        public async Task<ActionResult<ApiResponse<object>>> CanDeleteTag(int id)
        {
            try
            {
                var (canDelete, reason) = await _tagService.CanDeleteTagAsync(id);
                return Ok(ApiResponse<object>.SuccessResponse(new { CanDelete = canDelete, Reason = reason }));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error occurred while checking if tag {TagId} can be deleted.", id);
                return StatusCode(500, ApiResponse<object>.ErrorResponse(
                    "An error occurred while checking if tag can be deleted.", ex.Message));
            }
        }
        ...
        /// </summary>
        [HttpGet("unused")]
        public async Task<ActionResult<ApiResponse<IEnumerable<TagDto>>>> GetUnusedTags()
        {
            try
            {
                var tags = await _tagService.GetUnusedTagsAsync();
                return Ok(ApiResponse<IEnumerable<TagDto>>.SuccessResponse(tags, "Unused tags retrieved successfully."));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error occurred while getting unused tags.");
                return StatusCode(500, ApiResponse<IEnumerable<TagDto>>.ErrorResponse(
                    "An error occurred while retrieving unused tags.", ex.Message));
            }
        }

        /// <summary>
        /// Get most used tags
        /// GET: api/tag/most-used?top={topCount}
        /// </summary>
        [HttpGet("most-used")]
        public async Task<ActionResult<ApiResponse<IEnumerable<TagDto>>>> GetMostUsedTags([FromQuery] int top = 10)
        {
            try
            {
                var tags = await _tagService.GetMostUsedTagsAsync(top);
                return Ok(ApiResponse<IEnumerable<TagDto>>.SuccessResponse(
                    tags, $"Top {top} most used tags retrieved successfully."));
            }
            catch (ArgumentException ex)
            {
                _logger.LogWarning(ex, "Invalid top count provided.");
                return BadRequest(ApiResponse<IEnumerable<TagDto>>.ErrorResponse(ex.Message));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error occurred while getting most used tags.");
                return StatusCode(500, ApiResponse<IEnumerable<TagDto>>.ErrorResponse(
                    "An error occurred while retrieving most used tags.", ex.Message));
            }
        }
        ...
        /// </summary>
        [HttpGet("statistics")]
        public async Task<ActionResult<ApiResponse<TagStatisticsDto>>> GetTagStatistics()
        {
            try
            {
                var statistics = await _tagService.GetTagStatisticsAsync();
                return Ok(ApiResponse<TagStatisticsDto>.SuccessResponse(
                    statistics, "Tag statistics retrieved successfully."));
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error occurred while getting tag statistics.");
                return StatusCode(500, ApiResponse<TagStatisticsDto>.ErrorResponse(
                    "An error occurred while retrieving tag statistics.", ex.Message));
            }
        }
